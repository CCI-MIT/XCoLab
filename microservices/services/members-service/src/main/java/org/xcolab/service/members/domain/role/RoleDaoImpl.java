...
public class RoleDaoImpl implements RoleDao {
    ...
    public List<Role_> getMemberRoles(Long userId) {
        return this.dslContext.select()
                ...
                .where(USERS_ROLES.USER_ID.equal(userId)).fetchInto(Role_.class);
    }
    ...
    @Override
    public void assignMemberRole(Long userId, Long roleId) {
    ...
    public boolean memberHasRole(Long userId, Long roleId) {
        return this.dslContext.selectCount()
                .from(USERS_ROLES)
                .where(USERS_ROLES.ROLE_ID.eq(roleId)).and(USERS_ROLES.USER_ID.equal(userId))
    ...
    public boolean deleteMemberRole(long userId, long roleId) {
        return dslContext.deleteFrom(USERS_ROLES)
                .where(USERS_ROLES.USER_ID.eq(userId).and(USERS_ROLES.ROLE_ID.eq(roleId)))
                .execute() > 0;
    }
}
