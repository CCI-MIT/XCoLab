...
public class PhasePromotionHelper {
    ...
    public boolean proposalIsVisible(Proposal p) {
        ...
        try {
            ProposalContestPhaseAttribute attr = ProposalContestPhaseAttributeClient.getProposalContestPhaseAttribute(
    ...
    public ProposalContestPhaseAttribute getAttribute(long proposalId, String key) {
        try {
            return ProposalContestPhaseAttributeClient.getProposalContestPhaseAttribute(proposalId, phase.getContestPhasePK(), key);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public boolean hasProposalAlreadyBeenPromoted(Proposal p) {
        boolean hasProposalAlreadyBeenPromoted = ProposalContestPhaseAttributeClient.isProposalContestPhaseAttributeSetAndTrue(
                p.getProposalId(),
    ...
    public static void createProposalContestPhasePromotionDoneAttribute(long proposalId, long currentPhaseId) {
        ...
        if (currentPhaseId != 0) {
            ProposalContestPhaseAttributeClient.persistProposalContestPhaseAttribute(
                    proposalId,
                    currentPhaseId,
                    ProposalContestPhaseAttributeKeys.PROMOTE_DONE,
                    0l,
