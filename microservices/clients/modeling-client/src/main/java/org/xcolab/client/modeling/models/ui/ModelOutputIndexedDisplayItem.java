...
public class ModelOutputIndexedDisplayItem extends ModelOutputDisplayItem {
    ...
    public List<ModelOutputSeriesDisplayItem> getSeriesWithOutOfRangeError() {
        List<ModelOutputSeriesDisplayItem> ret = new ArrayList<>();
    ...
    public List<ModelOutputSeriesDisplayItem> getVisibleSeries() {
        if (visibleSeries == null) {
            ...
            visibleSeries = new ArrayList<>(
                    getSeries());
            for (Iterator<ModelOutputSeriesDisplayItem> items =
                    visibleSeries.iterator(); items.hasNext(); ) {
                ModelOutputSeriesDisplayItem item = items.next();
                _log.debug("{} is visible", item.getMetaData().getInternalName());
                if (!item.isVisible()) {
                    _log.debug("{} should not be visible", item.getMetaData().getInternalName());
                    items.remove();
                }
            }
        }
