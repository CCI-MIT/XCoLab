...
function initUserAutocomplete(idPostfix, canEdit) {
    ...
    jQuery('#userSelectorInput'+idPostfix).blur(function() {
        setTimeout(function() {
            ...
            // if (screenName == userNamePoints) {
            //     recipient = userIdPoints;
            // }
            // else {
        ...
        // don't navigate away from the field on tab when selecting an item
            ...
            .autocomplete({
                ...
                select: function( event, ui ) {
                    userNamePoints = ui.item.value;
                    userIdPoints = ui.item.id;
                    if (!jQuery('.listItem'+idPostfix+'-'+ui.item.id).length) {
                        //show container
                        jQuery("#userDistributionTable" + idPostfix).css("display", "block");
                        //add input
                        var htmlTemplate =
                            '<li class="listItem' + idPostfix + '-' + ui.item.id + '">'
                        ...
                        if (canEdit) {
                            htmlTemplate +=
                                '<span class="deleteListItem" data-point-id="' + idPostfix + '" data-item-id="' + idPostfix + '-' + ui.item.id + '">x</span>' +
                                '</label></li>'
                            ;
                        }
