...
/**
 * The base implementation of the messaging redirect link local service.
 *
 * <p>
...
public abstract class MessagingRedirectLinkLocalServiceBaseImpl
    implements MessagingRedirectLinkLocalService, IdentifiableBean {
    private static Log _log = LogFactoryUtil.getLog(MessagingRedirectLinkLocalServiceBaseImpl.class);
    ...
    public MessagingRedirectLink addMessagingRedirectLink(
        ...
        messagingRedirectLink = messagingRedirectLinkPersistence.update(messagingRedirectLink,
                false);
        ...
        if (indexer != null) {
            try {
                indexer.reindex(messagingRedirectLink);
    ...
    public void deleteMessagingRedirectLink(long redirectId)
        ...
        MessagingRedirectLink messagingRedirectLink = messagingRedirectLinkPersistence.remove(redirectId);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.delete(messagingRedirectLink);
            } catch (SearchException se) {
                if (_log.isWarnEnabled()) {
                    _log.warn(se, se);
                }
    ...
    public MessagingRedirectLink updateMessagingRedirectLink(
        MessagingRedirectLink messagingRedirectLink) throws SystemException {
        return updateMessagingRedirectLink(messagingRedirectLink, true);
    }
