...
/**
 * The base implementation of the contest phase type local service.
 *
 * <p>
...
public abstract class ContestPhaseTypeLocalServiceBaseImpl
    implements ContestPhaseTypeLocalService, IdentifiableBean {
    private static Log _log = LogFactoryUtil.getLog(ContestPhaseTypeLocalServiceBaseImpl.class);
    ...
    public ContestPhaseType addContestPhaseType(
        ...
        contestPhaseType = contestPhaseTypePersistence.update(contestPhaseType,
                false);
        ...
        if (indexer != null) {
            try {
                indexer.reindex(contestPhaseType);
    ...
    public void deleteContestPhaseType(long id)
        ...
        ContestPhaseType contestPhaseType = contestPhaseTypePersistence.remove(id);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.delete(contestPhaseType);
            } catch (SearchException se) {
                if (_log.isWarnEnabled()) {
                    _log.warn(se, se);
                }
    ...
    public ContestPhaseType updateContestPhaseType(
        ContestPhaseType contestPhaseType) throws SystemException {
        return updateContestPhaseType(contestPhaseType, true);
    }
