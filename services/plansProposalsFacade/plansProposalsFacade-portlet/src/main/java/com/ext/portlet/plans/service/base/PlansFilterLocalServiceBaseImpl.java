...
/**
 * The base implementation of the plans filter local service.
 *
 * <p>
 * This implementation exists only as a container for the default service methods generated by ServiceBuilder. All custom service methods should be put in {@link com.ext.portlet.plans.service.impl.PlansFilterLocalServiceImpl}.
 * </p>
 *
...
public abstract class PlansFilterLocalServiceBaseImpl
    implements PlansFilterLocalService, IdentifiableBean {
    private static Log _log = LogFactoryUtil.getLog(PlansFilterLocalServiceBaseImpl.class);
     ...
     * NOTE FOR DEVELOPERS:
     *
     * Never modify or reference this class directly. Always use {@link com.ext.portlet.plans.service.PlansFilterLocalServiceUtil} to access the plans filter local service.
     */
    ...
    public PlansFilter addPlansFilter(PlansFilter plansFilter)
        ...
        plansFilter = plansFilterPersistence.update(plansFilter, false);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.reindex(plansFilter);
    ...
    public void deletePlansFilter(PlansFilterPK plansFilterPK)
        ...
        PlansFilter plansFilter = plansFilterPersistence.remove(plansFilterPK);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.delete(plansFilter);
            } catch (SearchException se) {
                if (_log.isWarnEnabled()) {
                    _log.warn(se, se);
                }
    ...
    public PlansFilter updatePlansFilter(PlansFilter plansFilter)
        throws SystemException {
        return updatePlansFilter(plansFilter, true);
    }
    ...
    public PlansFilter updatePlansFilter(PlansFilter plansFilter, boolean merge)
        ...
        plansFilter = plansFilterPersistence.update(plansFilter, merge);
    ...
    public void afterPropertiesSet() {
        PersistedModelLocalServiceRegistryUtil.register("com.ext.portlet.plans.model.PlansFilter",
            plansFilterLocalService);
    }

    public void destroy() {
        PersistedModelLocalServiceRegistryUtil.unregister(
            "com.ext.portlet.plans.model.PlansFilter");
    }
