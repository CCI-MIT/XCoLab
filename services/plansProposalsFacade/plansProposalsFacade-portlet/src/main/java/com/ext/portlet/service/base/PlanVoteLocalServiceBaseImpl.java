...
/**
 * The base implementation of the plan vote local service.
 *
 * <p>
...
public abstract class PlanVoteLocalServiceBaseImpl
    ...
    private static Log _log = LogFactoryUtil.getLog(PlanVoteLocalServiceBaseImpl.class);
    ...
    public PlanVote addPlanVote(PlanVote planVote) throws SystemException {
        ...
        planVote = planVotePersistence.update(planVote, false);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.reindex(planVote);
    ...
    public void deletePlanVote(PlanVotePK planVotePK)
        ...
        PlanVote planVote = planVotePersistence.remove(planVotePK);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.delete(planVote);
            } catch (SearchException se) {
                if (_log.isWarnEnabled()) {
                    _log.warn(se, se);
                }
    ...
    public PlanVote updatePlanVote(PlanVote planVote) throws SystemException {
        return updatePlanVote(planVote, true);
    }
    ...
    public PlanVote updatePlanVote(PlanVote planVote, boolean merge)
        ...
        planVote = planVotePersistence.update(planVote, merge);
