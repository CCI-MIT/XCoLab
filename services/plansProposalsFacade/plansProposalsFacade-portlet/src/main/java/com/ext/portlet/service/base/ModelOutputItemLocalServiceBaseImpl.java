...
/**
 * The base implementation of the model output item local service.
 *
 * <p>
...
public abstract class ModelOutputItemLocalServiceBaseImpl
    implements ModelOutputItemLocalService, IdentifiableBean {
    private static Log _log = LogFactoryUtil.getLog(ModelOutputItemLocalServiceBaseImpl.class);
    ...
    public ModelOutputItem addModelOutputItem(ModelOutputItem modelOutputItem)
        ...
        modelOutputItem = modelOutputItemPersistence.update(modelOutputItem,
                false);
        ...
        if (indexer != null) {
            try {
                indexer.reindex(modelOutputItem);
    ...
    public void deleteModelOutputItem(long modelOutputItemModifierPK)
        ...
        if (indexer != null) {
            try {
                indexer.delete(modelOutputItem);
            } catch (SearchException se) {
                if (_log.isWarnEnabled()) {
                    _log.warn(se, se);
                }
    ...
    public ModelOutputItem updateModelOutputItem(
        ...
        return updateModelOutputItem(modelOutputItem, true);
    }
