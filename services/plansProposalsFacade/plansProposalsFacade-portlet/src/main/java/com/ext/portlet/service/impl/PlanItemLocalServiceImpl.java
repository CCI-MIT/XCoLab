 ...
 * {@link com.ext.portlet.service.PlanItemLocalService} interface.
 * 
 * <p>
...
public class PlanItemLocalServiceImpl extends PlanItemLocalServiceBaseImpl {
	 ...
	 * com.ext.portlet.service.PlanItemLocalServiceUtil} to access the plan item
	 * local service.
	 */

	private static Log _log = LogFactoryUtil
			.getLog(PlanItemLocalServiceImpl.class);
	 ...
	 */
	public static final String[] DEFAULT_CATEGORY_GUEST_PERMISSIONS = {
			ActionKeys.VIEW, ActionKeys.SUBSCRIBE };
	...
	public PlanItem createPlan(ContestPhase phase, Long authorId)
		...
		long planItemId = CounterLocalServiceUtil.increment(PlanItem.class
				.getName());
		long planId = CounterLocalServiceUtil.increment(PlanItem.class
				.getName() + PLAN_ID_NAME_SUFFIX);
		long planTypeId = ContestPhaseLocalServiceUtil.getContest(phase)
				.getPlanTypeId();
		...
		PlanMeta meta = PlanMetaLocalServiceUtil.createPlanMeta(planItem,
				planTypeId);
		PlanType type = PlanTypeLocalServiceUtil.getPlanType(planTypeId);
		Simulation defaultmodel = PlanTypeLocalServiceUtil
				.getDefaultModel(type);
		if (defaultmodel != null) {
			meta.setModelId(defaultmodel.getId());
		} else {
			List<Simulation> models = PlanTypeLocalServiceUtil
	 ...
	 */
	private PlanItem createPlan(Long planTypeId, Long authorId)
			throws SystemException, PortalException {
		long planItemId = CounterLocalServiceUtil.increment(PlanItem.class
				.getName());
		long planId = CounterLocalServiceUtil.increment(PlanItem.class
	...
	public PlanItem createPlan(PlanItem basePlan, Long authorId)
			throws SystemException, PortalException {
	...
	public PlanItem createPlan(PlanItem basePlan, ContestPhase contestPhase,
			Long authorId) throws SystemException, PortalException {
		long type = getPlanTypeId(basePlan);
		if (ContestLocalServiceUtil.getPlanType(
				ContestPhaseLocalServiceUtil.getContest(contestPhase))
				.getPlanTypeId() != type) {
			_log.error("Cannot create plan of type " + type
					+ " for contest phase "
					+ ContestPhaseLocalServiceUtil.getName(contestPhase));
		}

		// PlanItem plan = createPlan(ContestLocalServiceUtil.getPlanType(
		// ContestPhaseLocalServiceUtil.getContest(contestPhase)).getPlanTypeId(),
		// authorId);
		...
		// create related entities, plan description, meta, model run
		PlanDescription description = PlanDescriptionLocalServiceUtil
				.createPlanDescription(planItem, name);
		...
		PlanModelRunLocalServiceUtil.createPlanModelRun(planItem);
		PlanMeta planMeta = PlanMetaLocalServiceUtil.createPlanMeta(planItem,
				ContestPhaseLocalServiceUtil.getContest(contestPhase)
						.getPlanTypeId());
		...
		// PlanDescription description =
		// PlanDescriptionLocalServiceUtil.getCurrentForPlan(planItem);
		PlanModelRun planModelRun = PlanModelRunLocalServiceUtil
				.getCurrentForPlan(planItem);
		PlanPositions planPositions = PlanPositionsLocalServiceUtil
				.getCurrentForPlan(planItem);
		...
		if (sections != null && !sections.isEmpty()) {
			for (PlanSection section : sections) {
				...
				setSectionContent(planItem, def, section.getContent(),
						new ArrayList<Long>(), authorId);
			}
		}
		...
		try {
			if (scenarioId != null
					&& scenarioId > 0
					&& CollaboratoriumModelingService.repository().getScenario(
							scenarioId) != null) {
			...
			_log.error("Can't find scenario with id: "
					+ getScenarioId(basePlan), e);
		}
		PlanModelRunLocalServiceUtil.store(planModelRun);

		// copy positions
		PlanPositionsLocalServiceUtil.setPositionsIds(planPositions,
				getPositionsIds(basePlan));
		...
		Attribute[] attributesToCopy = { Attribute.ABSTRACT,
				Attribute.LAST_MOD_DATE, Attribute.SCRAPBOOK,
				Attribute.SUBREGION, Attribute.REGION,
				Attribute.SEEKING_ASSISTANCE, Attribute.IS_PLAN_OPEN };

		for (Attribute attr : attributesToCopy) {
			PlanAttribute pa = getPlanAttribute(basePlan, attr.name());
			if (pa != null) {
				setAttribute(planItem, pa.getAttributeName(),
	...
	private void initPlan(PlanItem plan) throws PortalException,
			SystemException {
		...
		String groupName = rand.nextLong() + "_" + System.currentTimeMillis()
				+ "_" + getName(plan);
		Group group = null;
		try {
			group = GroupServiceUtil.addGroup(
					StringUtils.substring(groupName, 0, 80),
					String.format(DEFAULT_GROUP_DESCRIPTION,
							StringUtils.substring(groupName, 0, 80)),
					GroupConstants.TYPE_SITE_RESTRICTED, null, true, true,
					groupServiceContext);
		} catch (Exception e) {
		...
		categoryGroup.setUrl("/web/guest/plans/-/plans/contestId/"
				+ getContest(plan).getContestPK() + "/planId/"
				+ plan.getPlanId() + "#plans=tab:comments");

		DiscussionCategoryGroupLocalServiceUtil.store(categoryGroup);

		DiscussionCategory category = DiscussionCategoryGroupLocalServiceUtil
				.addCategory(categoryGroup, "General discussion", null,
	...
	public List<PlanItem> getPlansInContestPhase(long contestPhaseId)
			throws SystemException, PortalException {

		return this.getPlans(Collections.emptyMap(), Collections.emptyMap(),
				0L, contestPhaseId, 0, Integer.MAX_VALUE, "", "", false);
	}

	public PlanItem getPlan(Long planId) throws NoSuchPlanItemException,
			SystemException {
		return this.planItemFinder.findLatestVersion(planId);
	}

	public List<PlanItem> getPlans(Map sessionMap, Map requestMap,
			long planTypeId, long contestPhaseId, int start, int end,
			final String sortColumn, String sortDirection)
			throws SystemException, PortalException {
		return getPlans(sessionMap, requestMap, planTypeId, contestPhaseId,
				start, end, sortColumn, sortDirection, true);
	}
	...
	public List<PlanItem> getPlans(Map sessionMap, Map requestMap,
		...
		PlanType planType = planTypeId > 0 ? PlanTypeLocalServiceUtil
				.getPlanType(planTypeId) : null;
		ContestPhase phase = contestPhaseId > 0 ? ContestPhaseLocalServiceUtil
				.getContestPhase(contestPhaseId) : null;
		/*
		...
		if (phase != null) {
			try {
				plans = new ArrayList(planItemFinder.getPlansForPhase(phase
						.getContestPhasePK()));
			} catch (Throwable t) {
				t.printStackTrace();
				throw t;
			...
			for (PlanItem planItem : planItemFinder.getPlans()) {
				if (planType == null
						|| (getPlanTypeId(planItem) != null && getPlanTypeId(
								planItem).equals(planType.getPlanTypeId()))) {
					plans.add(planItem);
				}
			}
		}
		...
		if (planType == null && phase != null) {
			planType = ContestLocalServiceUtil
			...
			Collections.sort(plans, new Comparator<PlanItem>() {
				...
				public int compare(PlanItem arg0, PlanItem arg1) {
					try {
						...
						PlanAttribute plan1Attr = getPlanAttribute(arg0,
								sortColumn);
						PlanAttribute plan2Attr = getPlanAttribute(arg1,
								sortColumn);

						if (plan1Attr != null) {
							val1 = (Comparable) PlanAttributeLocalServiceUtil
									.getTypedValue(plan1Attr);
						}
						if (plan2Attr != null) {
							val2 = (Comparable) PlanAttributeLocalServiceUtil
									.getTypedValue(plan2Attr);
						...
						if (val1 == null || val2 == null) {
							try {
								Attribute attribute = Attribute
										.valueOf(sortColumn);
								val1 = (Comparable) attribute
										.calculateTypedValue(arg0);
								val2 = (Comparable) attribute
										.calculateTypedValue(arg1);
							} catch (Exception e) {
							}
						}
	...
	public List<PlanItem> getPlans(Map sessionMap, Map requestMap,
			PlanType planType, List<ContestPhase> phases, int start, int end,
			final String sortColumn, String sortDirection)
			throws SystemException, PortalException {
		return getPlans(sessionMap, requestMap, planType, phases, start, end,
				sortColumn, sortDirection, true);
	}
	...
	public List<PlanItem> getPlans(Map sessionMap, Map requestMap,
		...
		List<PlanItem> plans = new ArrayList<PlanItem>();
		for (PlanItem planItem : planItemFinder.getPlans()) {
			if ((planType == null || (getPlanTypeId(planItem) != null && getPlanTypeId(
					planItem).equals(planType.getPlanTypeId())))
					&& (getPlanMeta(planItem).getContestPhase() != 0 && phasesIds
							.contains(getPlanMeta(planItem).getContestPhase()))) {
				plans.add(planItem);
			}
		}
		...
		if (planType == null && phases != null && !phases.isEmpty()) {
			planType = ContestLocalServiceUtil
					.getPlanType(ContestPhaseLocalServiceUtil.getContest(phases
							.get(0)));
		}
		...
		Collections.sort(plans, new Comparator<PlanItem>() {
			...
			public int compare(PlanItem arg0, PlanItem arg1) {
				try {
					...
					Comparable val1 = (Comparable) (plan1Attr != null ? PlanAttributeLocalServiceUtil
							.getTypedValue(plan1Attr) : null);
					Comparable val2 = (Comparable) (plan2Attr != null ? PlanAttributeLocalServiceUtil
							.getTypedValue(plan2Attr) : null);
					if (val1 != null) {
						if (val2 != null) {
							return val1.compareTo(val2) * directionModifier;
						} else {
	...
	public List<PlanItem> applyFilters(Map sessionMap, Map requestMap,
			PlanType planType, List<PlanItem> plans) throws PortalException,
			SystemException {
		/*
		 * PlansUserSettings planUserSettings =
		 * PlansUserSettingsLocalServiceUtil.getPlanUserSettings(sessionMap,
		 * requestMap, planType); if (!planUserSettings.getFilterEnabled()) {
		 * return plans; }
		 * 
		 * List<PlanItem> filteredPlans = new ArrayList<PlanItem>();
		 * 
		 * for (PlanItem plan : plans) { boolean inFilteredSet = true; for
		 * (Attribute attribute : Attribute.values()) { if
		 * (!attribute.isInFilteredSet(planUserSettings, plan)) { inFilteredSet
		 * = false; break; } } if (inFilteredSet) { filteredPlans.add(plan); } }
		 * 
	...
	public List<PlanAttribute> getPlanAttributes(PlanItem plan)
			throws SystemException {
		return PlanAttributeLocalServiceUtil
				.getPlanAttributes(plan.getPlanId());
	}

	public PlanAttribute getPlanAttribute(PlanItem plan, String name)
			throws SystemException {
		return PlanAttributeLocalServiceUtil.findPlanAttribute(
				plan.getPlanId(), name);
	}
	...
	public void reIndex() throws SystemException {
		...
		/*
		 * for (PlanItem plan : PlanItemLocalServiceUtil.getPlans()) { try {
		 * Indexer.updateEntry(defaultCompanyId, plan); } catch (SearchException
		 * e) {
		 * _log.error("An exception has been thrown when reindexing plan with id: "
		 * + plan.getPlanId(), e); } }
		 */
	}
	...
	public void reIndex(long planId) throws SystemException {
		// reindex concrete plan
		/*
		 * try { PlanItem plan = PlanItemLocalServiceUtil.getPlan(planId);
		 * Indexer.updateEntry(defaultCompanyId, plan); } catch
		 * (NoSuchPlanItemException e) {
		 * _log.error("An exception has been thrown when reindexing plan with id: "
		 * + planId, e); } catch (SearchException e) {
		 * _log.error("An exception has been thrown when reindexing plan with id: "
		 * + planId, e); }
		 */
	}

	public List<PlanItem> findPlansForFocusArea(FocusArea fa)
			throws PortalException, SystemException {
		return findPlansForOntologyTerms(FocusAreaLocalServiceUtil.getTerms(fa));
	}

	public List<PlanItem> findPlansForOntologyTerms(OntologyTerm... terms)
			throws NoSuchPlanItemException, SystemException {
		return findPlansForOntologyTerms(Arrays.asList(terms));
	}

	public List<PlanItem> findPlansForOntologyTerms(List<OntologyTerm> terms)
			throws NoSuchPlanItemException, SystemException {
		Set<Long> ids = null;

		for (OntologyTerm term : terms) {
			List<Long> tmp = OntologyTermLocalServiceUtil.findTagedIdsForClass(
					term, PlanItem.class);
			if (ids == null) {
	...
	@Override
	public List<PlanItem> findPlansForOntologyTerms(OntologyTerm term)
			throws NoSuchPlanItemException, SystemException {
		return findPlansForOntologyTerms(new OntologyTerm[] { term });
	}

	public long countPlansByContestPhase(ContestPhase phase)
			throws SystemException, PortalException {
		return planItemFinder.countPlansByContestPhase(phase
				.getContestPhasePK());
	}

	public List<PlanItem> getPlansByContest(Long contestId)
			throws SystemException, PortalException {
		List<ContestPhase> phases = ContestPhaseLocalServiceUtil
				.getPhasesForContest(ContestLocalServiceUtil
						.getContest(contestId));
		List<PlanItem> ret = new ArrayList<PlanItem>();
		long counter = 0;
		for (ContestPhase phase : phases) {
			for (PlanItem plan : planItemFinder.getPlansForPhase(phase
					.getContestPhasePK())) {
				if (plan.getVersion() > 1
						&& !plan.getState().equals(EntityState.DELETED.name())) {
					ret.add(plan);
				}
			}
		}
	...
	public void planDeleted(PlanItem plan) throws PortalException,
			SystemException {
		planItemFinder.clearPhaseCache(getContestPhase(plan)
				.getContestPhasePK());
	...
	public void setDescription(PlanItem pi, String description,
			Long updateAuthorId) throws SystemException, PortalException {
		newVersion(pi, UpdateType.DESCRIPTION_UPDATED, updateAuthorId);

		PlanDescription planDescription = PlanDescriptionLocalServiceUtil
	...
	public void setName(PlanItem pi, String name, Long updateAuthorId)
			throws SystemException, PortalException {
	...
	public void setImage(PlanItem pi, Long imageId, Long updateAuthorId)
			throws SystemException, PortalException {
	...
	public void setPitch(PlanItem pi, String pitch, Long updateAuthorId)
	...
	public List<PlanDescription> getAllDescriptionVersions(PlanItem pi)
			throws SystemException {
	 ...
	 */
	public List<PlanDescription> getPlanDescriptions(PlanItem pi)
			throws SystemException {
		return PlanDescriptionLocalServiceUtil.getAllForPlan(pi);
	}
	...
	public Long getScenarioId(PlanItem pi) throws SystemException {
		return PlanModelRunLocalServiceUtil.getCurrentForPlan(pi)
				.getScenarioId();
	}

	public void setScenarioId(PlanItem pi, Long scenarioId, Long authorId)
			throws SystemException, PortalException {
		...
		for (PlanConstants.Attribute attribute : PlanConstants.Attribute
				.getPlanTypeAttributes(planType)) {
	...
	public void setModelId(PlanItem pi, Long simulationId, Long authorId)
		...
		PlanType planType = getPlanType(pi);
		List<Simulation> sims = PlanTypeLocalServiceUtil
		...
		if (!found) {
			throw new SystemException("Model id " + simulationId
					+ " not valid for planType");
		}

		newVersion(pi, UpdateType.MODEL_UPDATED, authorId);
		PlanMeta planMeta = PlanMetaLocalServiceUtil
				.createNewVersionForPlan(pi);
	...
	public List<PlanModelRun> getAllPlanModelRuns(PlanItem pi)
			throws SystemException {
	...
	public PlanType getPlanType(PlanItem pi) throws SystemException,
			PortalException {
		return PlanTypeLocalServiceUtil.getPlanType(getPlanTypeId(pi));
	}

	public Contest getContest(PlanItem pi) throws SystemException,
			PortalException {
		ContestPhase phase = getContestPhase(pi);
		return phase == null ? null : ContestPhaseLocalServiceUtil
				.getContest(phase);
	}

	public ContestPhase getContestPhase(PlanItem pi) throws SystemException,
			PortalException {
		Long phase = getPlanMeta(pi).getContestPhase();
		return phase == null ? null : ContestPhaseLocalServiceUtil
				.getContestPhase(phase);
	}

	public void setContestPhase(PlanItem pi, ContestPhase phase,
			Long updateAuthorId) throws SystemException {
	...
	public void setPlanTypeId(PlanItem pi, Long planTypeId, Long updateAuthorId)
			throws SystemException, PortalException {
		newVersion(pi, UpdateType.PLAN_TYPE_UPDATED, updateAuthorId);
	...
	public void setMBCategoryId(PlanItem pi, Long mbCategoryId,
			Long updateAuthorId) throws SystemException {
	...
	public void setCategoryGroupId(PlanItem pi, Long categoryGroupId,
			Long updateAuthorId) throws SystemException {
		newVersion(pi, UpdateType.MB_GROUP_UPDATED, updateAuthorId);
	...
	public void setPlanGroupId(PlanItem pi, Long groupId, Long updateAuthorId)
			throws SystemException {
	...
	public void setAuthorId(PlanItem pi, Long authorId, Long updateAuthorId)
			throws SystemException {
		newVersion(pi, UpdateType.PLAN_GROUP_UPDATED, updateAuthorId);
	...
	public String getCreator(PlanItem pi) throws PortalException,
			SystemException {
	...
	public void setOpen(PlanItem pi, boolean open, Long updateAuthorId)
			throws SystemException {
		if (open) {
	...
	public void setStatus(PlanItem pi, String status, Long updateAuthorId)
			throws SystemException {
	...
	public void setPositions(PlanItem pi, List<Long> positionsIds,
			Long updateAuthorId) throws PortalException, SystemException {
	...
	public List<PlanPositions> getAllPositionsVersions(PlanItem pi)
			throws SystemException {
	 ...
	 * VOTES
	 */
	public boolean hasUserVoted(PlanItem pi, Long userId)
			throws PortalException, SystemException {
		try {
			PlanVote vote = PlanVoteLocalServiceUtil.getPlanVote(userId,
					getContest(pi).getContestPK());
	...
	public void vote(PlanItem pi, Long userId) throws PortalException,
			SystemException {
		if (PlanVoteLocalServiceUtil.voteForPlan(pi.getPlanId(), userId)) {
			PlanMetaLocalServiceUtil.vote(PlanMetaLocalServiceUtil
					.getCurrentForPlan(pi));
		}
		updateAttribute(pi, Attribute.VOTES);
	}

	public void unvote(PlanItem pi, Long userId) throws PortalException,
			SystemException {
		if (PlanVoteLocalServiceUtil.unvote(userId, getContest(pi)
				.getContestPK())) {
			PlanMetaLocalServiceUtil.unvote(PlanMetaLocalServiceUtil
					.getCurrentForPlan(pi));
		}
		updateAttribute(pi, Attribute.VOTES);
	}

	private PlanItem newVersion(PlanItem pi, UpdateType updateType,
			Long updateAuthorId) throws SystemException {
		PlanItem latestVersion = pi;
		try {
			/*
			 * 
			 * // i don't think that this is a problem if
			 * (!latestVersion.equals()) { throw new SystemException(
		...
		setAttribute(pi, PlanConstants.Attribute.LAST_MOD_DATE,
				String.valueOf(pi.getUpdated()));
	 ...
	 */
	public void updateAttribute(PlanItem pi, String attributeName)
			throws SystemException {
	 ...
	 *             in case of any error
	 */
	private void updateAttribute(PlanItem pi, Attribute attribute)
			throws SystemException {
		String value = attribute.calculateValue(pi).toString();
		PlanAttribute att = PlanAttributeLocalServiceUtil.findPlanAttribute(
				pi.getPlanId(), attribute.name());
		if (att != null) {
			...
			PlanAttributeLocalServiceUtil.addPlanAttribute(pi.getPlanId(),
	 ...
	 *             in case of any error
	 */
	private void setAttribute(PlanItem pi, Attribute attribute, String value)
			throws SystemException {
		PlanAttribute att = PlanAttributeLocalServiceUtil.findPlanAttribute(
				pi.getPlanId(), attribute.name());
		if (att != null) {
	 ...
	 *             in case of any error
	 */
	private String getAttribute(PlanItem pi, Attribute attribute)
			throws SystemException {
		PlanAttribute att = PlanAttributeLocalServiceUtil.findPlanAttribute(
				pi.getPlanId(), attribute.name());
		if (att != null) {
	...
	public List<MembershipRequest> getMembershipRequests(PlanItem pi)
			throws SystemException {
		return MembershipRequestLocalServiceUtil
				.search(getPlanGroupId(pi),
						MembershipRequestConstants.STATUS_PENDING, 0,
						Integer.MAX_VALUE);
	}

	public void addMembershipRequest(PlanItem pi, Long userId, String comments)
			throws PortalException, SystemException {
		PlanTeamHistoryLocalServiceUtil.newHistoryItem(pi.getPlanId(), userId,
				PlanTeamActions.MEMBERSHIP_REQUESTED.name(), userId);
		MembershipRequestLocalServiceUtil.addMembershipRequest(userId,
				getPlanGroupId(pi), comments, null);
	}

	public void dennyMembershipRequest(PlanItem pi, Long userId,
			MembershipRequest request, String reply, Long updateAuthorId)
			throws PortalException, SystemException {
		PlanTeamHistoryLocalServiceUtil.newHistoryItem(pi.getPlanId(), userId,
				PlanTeamActions.MEMBERSHIP_DECLEINED.name(), updateAuthorId);
		MembershipRequestLocalServiceUtil.updateStatus(userId,
				request.getMembershipRequestId(), reply,
				MembershipRequestConstants.STATUS_DENIED, false, null);
	}

	public void approveMembershipRequest(PlanItem pi, Long userId,
			MembershipRequest request, String reply, Long updateAuthorId)
			throws PortalException, SystemException {
		PlanTeamHistoryLocalServiceUtil.newHistoryItem(pi.getPlanId(), userId,
	 ...
	 */
	public void publish(PlanItem pi, Long updateAuthorId)
			throws SystemException, PortalException {
	...
	public void delete(PlanItem pi, Long updateAuthorId)
			throws SystemException, PortalException {
		...
		/*
		 * try { Indexer.deleteEntry(10112L, getPlanId()); } catch
		 * (SearchException e) { _log.error("can't remove plan " + getPlanId() +
		 * " from search index", e); }
		 */
	...
	public User getUpdateAuthor(PlanItem pi) throws PortalException,
			SystemException {
	...
	public PlanFan addFan(PlanItem pi, Long userId) throws SystemException {
		if (!isUserAFan(pi, userId)) {
			PlanFan planFan = PlanFanLocalServiceUtil.addFan(pi.getPlanId(),
					userId);
			setAttribute(pi, Attribute.SUPPORTERS,
					String.valueOf(PlanFanLocalServiceUtil
							.countPlanFansForPlan(pi.getPlanId())));
			return planFan;
		}
		return null;
	}
	...
	public boolean isUserAFan(PlanItem pi, Long userId) throws SystemException {
		try {
			return PlanFanLocalServiceUtil.getPlanFanByPlanIdUserId(
					pi.getPlanId(), userId) != null;
		} catch (NoSuchPlanFanException e) {
			return false;
		}
	}

	public boolean isUserAMember(PlanItem pi, Long userId)
			throws SystemException {
		return GroupLocalServiceUtil.hasUserGroup(userId, getPlanGroupId(pi));
	}

	public boolean hasUserRequestedMembership(PlanItem pi, Long userId)
			throws SystemException {
		try {
			PlanTeamHistory action = PlanTeamHistoryLocalServiceUtil
					.getLastUserActionInPlan(pi.getPlanId(), userId);
			if (action.getAction().equals(
					PlanTeamActions.MEMBERSHIP_REQUESTED.name())) {
				return true;
			}
	...
	public boolean isAdmin(PlanItem pi, Long userId) throws PortalException,
			SystemException {
		return UserGroupRoleLocalServiceUtil.hasUserGroupRole(userId,
				getPlanGroupId(pi), RoleConstants.SITE_ADMINISTRATOR)
				|| isOwner(pi, userId);
	}

	public boolean isOwner(PlanItem pi, Long userId) throws PortalException,
			SystemException {
		return UserGroupRoleLocalServiceUtil.hasUserGroupRole(userId,
				getPlanGroupId(pi), RoleConstants.SITE_OWNER);
	}

	public void setUserPermission(PlanItem pi, Long userId,
			String userPermission, Long updateAuthorId) throws SystemException,
			PortalException {
		PlanUserPermission userPerm = PlanUserPermission
				.valueOf(userPermission);

		Group group = GroupLocalServiceUtil.getGroup(getPlanGroupId(pi));
		UserGroupRoleLocalServiceUtil.deleteUserGroupRoles(userId,
				new long[] { getPlanGroupId(pi) });
		...
		Role role = RoleLocalServiceUtil
				.getRole(group.getCompanyId(), roleName);
		UserGroupRoleLocalServiceUtil.addUserGroupRoles(userId,
				getPlanGroupId(pi), new long[] { role.getRoleId() });
		PlanTeamHistoryLocalServiceUtil.newHistoryItem(pi.getPlanId(), userId,
				PlanTeamActions.PERMISSIONS_CHANGED.name(), userPermission,
				updateAuthorId);
	}

	public void removeMember(PlanItem pi, Long userId, Long updateAuthorId)
			throws SystemException, PortalException {
		UserLocalServiceUtil.unsetGroupUsers(getPlanGroupId(pi),
				new long[] { userId }, null);
		PlanTeamHistoryLocalServiceUtil.newHistoryItem(pi.getPlanId(), userId,
				PlanTeamActions.MEMBER_REMOVED.name(), updateAuthorId);
	}

	public void joinIfNotAMember(PlanItem pi, Long userId)
			throws SystemException, PortalException {
		if (!isUserAMember(pi, userId)) {
			GroupLocalServiceUtil.addUserGroups(userId,
					new long[] { getPlanGroupId(pi) });
			PlanTeamHistoryLocalServiceUtil.newHistoryItem(pi.getPlanId(),
					userId, PlanTeamActions.MEMBERSHIP_APPROVED.name(), userId);
		}

	}

	public void setSeekingAssistance(PlanItem pi, boolean seekingAssistance)
			throws SystemException {
		setAttribute(pi, Attribute.SEEKING_ASSISTANCE,
				String.valueOf(seekingAssistance));
	}
	...
	private void updateSearchIndex(PlanItem pi) throws SearchException,
			SystemException {
		// Indexer.updateEntry(10112L, this);

	}

	public DiscussionCategoryGroup getDiscussionCategoryGroup(PlanItem pi)
			throws PortalException, SystemException {
		return DiscussionCategoryGroupLocalServiceUtil
				.getDiscussionCategoryGroup(getCategoryGroupId(pi));

	}

	public PlanItem promote(PlanItem pi, User user) throws SystemException,
			PortalException {
		ContestPhase contestPhase = ContestPhaseLocalServiceUtil
				.getNextContestPhase(getContestPhase(pi));
	...
	public boolean getPromoted(PlanItem pi) throws SystemException {
		Boolean promoted = PlanMetaLocalServiceUtil.getCurrentForPlan(pi)
				.getPromoted();
		return promoted != null ? promoted : false;

	}

	public int getCommentsCount(PlanItem pi) throws PortalException,
			SystemException {
		return DiscussionCategoryGroupLocalServiceUtil
				.getCommentsCount(getDiscussionCategoryGroup(pi));
	}
	...
	public void setRibbonText(PlanItem pi, String ribbonText)
			throws SystemException {
		setAttribute(pi, Attribute.PLAN_RIBBON_TEXT, String.valueOf(ribbonText));
	}

	public void setAttribute(PlanItem pi, String attributeName, String value)
			throws SystemException {
		setAttribute(pi, Attribute.valueOf(attributeName), value);
	}

	public void removeAttribute(PlanItem pi, String attributeName)
			throws SystemException {
		PlanAttribute attr = PlanAttributeLocalServiceUtil.findPlanAttribute(
				pi.getPlanId(), attributeName);
	...
	public PlanTemplate getPlanTemplate(PlanItem pi) throws PortalException,
			SystemException {
		return ContestLocalServiceUtil.getPlanTemplate(getContest(pi));
	}

	public List<PlanSection> getPlanSections(PlanItem pi)
			throws PortalException, SystemException {
		PlanTemplate tmpl = getPlanTemplate(pi);
		if (tmpl != null) {
			List<PlanSection> ret = new ArrayList<PlanSection>();

			for (PlanSectionDefinition psd : PlanTemplateLocalServiceUtil
					.getSections(tmpl)) {
				ret.add(PlanSectionLocalServiceUtil.getForPlanSectionDef(pi,
						psd));
			}
			return ret;
		}
	...
	public void setSectionContent(PlanItem pi, PlanSectionDefinition psd,
			String content, List<Long> referencedPlans, Long updateAuthorId)
			throws SystemException, PortalException {
		Document contentDoc = Jsoup.parse(content);
		String[] forbiddenElements = new String[] { "body body","script", "head",
				"input", "form", "textarea", "link" };
		for(String s : forbiddenElements) {
			contentDoc.select(s).remove();
		}
		Element body = contentDoc.select("body").first();
		for(org.jsoup.nodes.Attribute a : body.attributes()) {
			body.attributes().remove(a.getKey());
		}
		//all links open in new window
		for(Element a : contentDoc.select("a")) {
			a.attr("target", "_blank");
			//remove JS links
			if(a.attr("href").startsWith("javascript:")) {
				a.remove();
			}
		}
		//remove onclick etc
		for(Element e : contentDoc.getAllElements()) {
	...
	public List<PlanSection> getAllPlanSections(PlanItem pi,
			PlanSectionDefinition psd) throws SystemException {
	...
	public Integer getRibbon(PlanItem pi) throws SystemException {
		PlanAttribute attr = getPlanAttribute(pi,
				PlanConstants.Attribute.PLAN_RIBBON.name());
		try {
			return attr != null && attr.getAttributeValue() != null
					&& attr.getAttributeValue().trim().length() > 0 ? Integer
	...
	public void revertTo(PlanItem pi, Long updateAuthorId)
			throws SystemException, PortalException {

		PlanDescription oldDesc = PlanDescriptionLocalServiceUtil
				.getForPlan(pi);
		...
		PlanDescriptionLocalServiceUtil.createNewVersionForPlanFrom(pi,
				oldDesc, true);
		PlanModelRunLocalServiceUtil.createNewVersionForPlanFrom(pi, oldAi,
				true);
	...
	public void setTagsHover(PlanItem pi, String tagsHover)
			throws SystemException {
	...
	public void promotePlans(long sourcePhasePk, long destPhasePk)
			throws PortalException, SystemException {
		ContestPhase sourcePhase = ContestPhaseLocalServiceUtil
				.getContestPhase(sourcePhasePk);
		ContestPhase targetPhase = ContestPhaseLocalServiceUtil
				.getContestPhase(destPhasePk);

		if (ContestPhaseLocalServiceUtil.getContest(targetPhase)
				.getPlanTypeId() != ContestPhaseLocalServiceUtil.getContest(
				sourcePhase).getPlanTypeId()) {
			throw new PortalException("Phases are incompatible, can't promote");
		} else {
			List<PlanItem> planCopyItems = new ArrayList<PlanItem>();
			promotePlans(ContestPhaseLocalServiceUtil.getPlans(sourcePhase),
					destPhasePk);
		}
	}

	public void promotePlans(List<PlanItem> plansToBeCopied, long destPhasePk)
			throws PortalException, SystemException {
		ContestPhase targetPhase = ContestPhaseLocalServiceUtil
				.getContestPhase(destPhasePk);
...
/*			// copy votes
			...
			DiscussionCategoryGroupLocalServiceUtil.copyEverything(dcg,
			...
			UserLocalServiceUtil
					.addGroupUsers(getPlanGroupId(newPlan), userIds);
			/*
			...
			DynamicQuery query = DynamicQueryFactoryUtil
					.forClass(ActivitySubscription.class);

			ClassName cn = ClassNameLocalServiceUtil
					.getClassName(PlanItem.class.getName());
			Criterion criterionClassNameId = RestrictionsFactoryUtil.eq(
					"classNameId", cn.getClassNameId());
			Criterion criterionClassPK = RestrictionsFactoryUtil.eq("classPK",
					plan.getPlanId());
			query.add(RestrictionsFactoryUtil.and(criterionClassNameId,
					criterionClassPK));

			for (Object subscriptionObj : ActivitySubscriptionLocalServiceUtil
					.dynamicQuery(query)) {
				ActivitySubscription subscription = (ActivitySubscription) subscriptionObj;

				ActivitySubscriptionLocalServiceUtil.addSubscription(
						PlanItem.class, newPlan.getPlanId(), 0, "",
						subscription.getReceiverId());
			}

			// copy subscriptions for comments
			query = DynamicQueryFactoryUtil
					.forClass(ActivitySubscription.class);

			cn = ClassNameLocalServiceUtil
					.getClassName(DiscussionCategoryGroup.class.getName());
			criterionClassNameId = RestrictionsFactoryUtil.eq("classNameId",
					cn.getClassNameId());
			criterionClassPK = RestrictionsFactoryUtil.eq("classPK",
					PlanItemLocalServiceUtil.getCategoryGroupId(plan));
			query.add(RestrictionsFactoryUtil.and(criterionClassNameId,
					criterionClassPK));
