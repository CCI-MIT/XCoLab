...
/**
 * The base implementation of the messaging message local service.
 *
 * <p>
...
public abstract class MessagingMessageLocalServiceBaseImpl
    implements MessagingMessageLocalService, IdentifiableBean {
    private static Log _log = LogFactoryUtil.getLog(MessagingMessageLocalServiceBaseImpl.class);
    ...
    public MessagingMessage addMessagingMessage(
        ...
        messagingMessage = messagingMessagePersistence.update(messagingMessage,
                false);
        ...
        if (indexer != null) {
            try {
                indexer.reindex(messagingMessage);
    ...
    public void deleteMessagingMessage(long messageId)
        ...
        MessagingMessage messagingMessage = messagingMessagePersistence.remove(messageId);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.delete(messagingMessage);
            } catch (SearchException se) {
                if (_log.isWarnEnabled()) {
                    _log.warn(se, se);
                }
    ...
    public MessagingMessage updateMessagingMessage(
        MessagingMessage messagingMessage) throws SystemException {
        return updateMessagingMessage(messagingMessage, true);
    }
