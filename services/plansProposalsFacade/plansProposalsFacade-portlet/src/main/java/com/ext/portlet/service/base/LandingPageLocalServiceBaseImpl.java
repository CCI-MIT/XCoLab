...
/**
 * The base implementation of the landing page local service.
 *
 * <p>
...
public abstract class LandingPageLocalServiceBaseImpl
    implements LandingPageLocalService, IdentifiableBean {
    private static Log _log = LogFactoryUtil.getLog(LandingPageLocalServiceBaseImpl.class);
    ...
    public LandingPage addLandingPage(LandingPage landingPage)
        ...
        landingPage = landingPagePersistence.update(landingPage, false);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.reindex(landingPage);
    ...
    public void deleteLandingPage(long id)
        ...
        LandingPage landingPage = landingPagePersistence.remove(id);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.delete(landingPage);
            } catch (SearchException se) {
                if (_log.isWarnEnabled()) {
                    _log.warn(se, se);
                }
    ...
    public LandingPage updateLandingPage(LandingPage landingPage)
        throws SystemException {
        return updateLandingPage(landingPage, true);
    }
    ...
    public LandingPage updateLandingPage(LandingPage landingPage, boolean merge)
        ...
        landingPage = landingPagePersistence.update(landingPage, merge);
