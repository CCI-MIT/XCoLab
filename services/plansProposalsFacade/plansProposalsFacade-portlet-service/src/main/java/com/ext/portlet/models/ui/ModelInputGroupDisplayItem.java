...
public class ModelInputGroupDisplayItem extends ModelInputDisplayItem implements Serializable {
    ...
    private static final Log _log = LogFactoryUtil.getLog(ModelInputGroupDisplayItem.class);
    ...
    public ModelInputGroupDisplayItem(ModelInputGroup group) throws SystemException, IOException {
        super(ModelInputGroupLocalServiceUtil.getModel(group),
                ModelInputGroupLocalServiceUtil.getMetaData(group));
        this.group = group;
        populateChildren();
    }
    ...
    public ModelInputDisplayItem addDisplayItem(MetaData d, ModelInputWidgetType type)
        ...
        if (!knownmd.contains(d)) {
            ModelInputIndividualDisplayItem item =
                    ModelInputIndividualDisplayItem.create(getSimulation(), d, type);
    ...
    public void addChildGroup(ModelInputGroupDisplayItem group)
            throws IllegalUIConfigurationException, SystemException, IOException {
        ...
        try {
            return group.getName() == null || group.getName().trim().equals("") ?
                    ModelInputGroupLocalServiceUtil.getMetaData(group) == null ?
                            null : ModelInputGroupLocalServiceUtil.getMetaData(group).getName()
                    : group.getName();
    ...
    public String getDescription() {
        try {
            return group.getDescription() == null || group.getDescription().trim().equals("") ?
                    ModelInputGroupLocalServiceUtil.getMetaData(group) == null ?
                            null
                            : ModelInputGroupLocalServiceUtil.getMetaData(group).getDescription()
    ...
    public void setParentGroupId(Long groupId)
        ...
        setParent(new ModelInputGroupDisplayItem(
                ModelInputGroupLocalServiceUtil.getModelInputGroup(groupId)));
    }
}
