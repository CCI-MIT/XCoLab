...
public class ContestCreationNotification extends EmailNotification {
     ...
     */
    private static final String COTNEST_CREATION_SUCCESS_NOTIFICATION_SUBJECT = "You created a createdContest!  Here's what's next...";
    /**
     * The body template for the proposal creation success notification
     */
    // TODO check message
    private static final String CONTEST_CREATION_SUCCESS_NOTIFICATION_BODY_TEMPLATE = "Dear FIRSTNAME_PLACEHOLDER,<br/>" +
            "<br/>Congratulations!  You recently created the contest, PROPOSAL_PLACEHOLDER on the Climate CoLab." +
            "<br/><br/>" +
            "<b>What's next:</b>" +
            "<ul>" +
            ...
            "<li><b>The YEAR_PLACEHOLDER createdContest deadline is DEADLINE_PLACEHOLDER</b>.  You can edit your proposal as many times as you want before then, but make sure your final " +
    ...
    private static final String CONTEST_SHARE_TEXT = "I just created a new createdContest at the Climate CoLab. Check it out!";
    private static final String CONTEST_SHARE_TITLE = "New createdContest at @ClimateCoLab";
    ...
    public void sendEmailNotification() throws PortalException, SystemException {
        String subject = COTNEST_CREATION_SUCCESS_NOTIFICATION_SUBJECT;
    ...
    private String populateBodyString() throws PortalException, SystemException {
        String body = StringUtil.replace(COTNEST_CREATION_SUCCESS_NOTIFICATION_SUBJECT, FIRSTNAME_PLACEHOLDER, getContestlAuthor(createdContest).getFirstName());
        body = StringUtil.replace(body, CONTEST_PLACEHOLDER, getContestLink(createdContest));
        //body = StringUtil.replace(body, TWITTER_PLACEHOLDER, getProposalTwitterShareLink(createdContest, createdProposal, PROPOSAL_SHARE_TEXT));
        //body = StringUtil.replace(body, FACEBOOK_PLACEHOLDER, getProposalFacebookShareLink(createdContest, createdProposal));
        //body = StringUtil.replace(body, PINTEREST_PLACEHOLDER, getProposalPinterestShareLink(createdContest, createdProposal, PROPOSAL_SHARE_TEXT));
        //body = StringUtil.replace(body, LINKEDIN_PLACEHOLDER, getProposalLinkedInShareLink(createdContest, createdProposal, PROPOSAL_SHARE_TITLE, PROPOSAL_SHARE_TEXT));
        ...
        if (Validator.isNull(createdContest.getCreated())) {
            ...
            body = StringUtil.replace(body, YEAR_PLACEHOLDER, yearFormat.format(createdContest.getCreated()));
        }

        DateFormat customDateFormat = new SimpleDateFormat("MMMM dd, HH:mm:ss a", Locale.US);
        // This should never happen when the createdContest phases are set up properly
