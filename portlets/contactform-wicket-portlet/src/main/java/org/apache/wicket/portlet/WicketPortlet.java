 ...
 * for Wicket, while it's actually running as a Portlet.
 * <p/>
 * <p/>
 * It receives a portlet request and dispatches to a a Wicket filter; wrapping the servlet context, request and response
 * objects; intercepts response writing (especially urls and redirects) and rewrites and adapts the output to
 ...
 * The WicketPortlet maintains a parameter for the current Wicket page URL being requested as a URL parameter, based
 * against the filter path (e.g. fully qualified to the context path). When a request (action, render or direct
 * resource/ajax call) is received by the WicketPortlet, it dispatches it to Wicket core as a filter request using the
...
public class WicketPortlet extends GenericPortlet {
     ...
     * <p/>
     * <p/>
     * The actual Wicket URLs generated by Wicket are passed around in portal URLs, encoded by the portal (as a URL
     * parameter of this name). The Wicket URL is later decoded on subsequent requests, from the portal URL, so that we
     * know where to route the request, once it's passed out of the 'portal' realm and into the 'Wicket' realm.
     * <p/>
     * <p/>
     * This is also used in generating links, as the links have to portal encoded links, but must also still contain the
     * original wicket url for use by Wicket
     * <p/>
     * <p/>
     */
    ...
    /**
     * A collection of the default URLs for the different view modes of the
    ...
    /**
     * Delegates to
     * {@link #processRequest(javax.portlet.PortletRequest, javax.portlet.PortletResponse, org.apache.wicket.portlet.WicketPortlet.PageType)}
     */
    protected void doCustom(final RenderRequest request, final RenderResponse response) throws PortletException,
            IOException {
     ...
     */
    @Override
    protected void doEdit(final RenderRequest request, final RenderResponse response) throws PortletException,
            IOException {
     ...
     */
    @Override
    protected void doHelp(final RenderRequest request, final RenderResponse response) throws PortletException,
            IOException {
    ...
    /**
     * @param pageType the mode of the portlet page, e.g. VIEW, EDIT etc...
     ...
     * <p/>
     * <p/>
     * An "attribute" is a Java object name/value pair passed only through the internal JavaServer processes. (I.e. it
     * can come from a JSP or servlet but not an HTML page.) Its value is an Object.
     * 
     * @param request
     * @param defaultPage url of the default page
     * @return the Wicket URL from within the specified request
     */
    protected String getWicketURL(final PortletRequest request, final String defaultPage) {
        String wicketURL;
        ...
        if ((wicketURL != null) && (requestParameters != null) && (!requestParameters.isEmpty())
                && (request instanceof ResourceRequest)) {
            /*
             * Many Javascript based components append parameters directly to the URL, so they are not part of the '_wu'
             * or 'resourceId' parameter. Wicket can access these parameters, but they are not present in the
             * query-string, so Wicket identifies them as POST parameters.
             */
     ...
     * {@link #processRequest(javax.portlet.PortletRequest, javax.portlet.PortletResponse, org.apache.wicket.portlet.WicketPortlet.PageType)}
     * <p/>
     * Stores the {@link ActionResponse} so that PortletEventService#broadcast can send events using
     * {@link ActionResponse#setEvent}
     */
    @Override
    public void processAction(final ActionRequest request, final ActionResponse response) throws PortletException,
            IOException {
     ...
     * The recorded url is then used in by wicket in the subsequent 'VIEW'
     ...
     * @throws IOException
     * @see IRequestCycleSettings.RenderStrategy#REDIRECT_TO_RENDER
     */
    ...
    private void processMimeResponseRequest(String wicketURL, final PortletRequest request,
        ...
        /*
         * keep looping until wicket processing does not result in a redirect (redirects have to be caught, and fed back
         * into Wicket as we only want the portlet redirected, not the entire page of course.
         */
        while (true) {
            rd = getPortletContext().getRequestDispatcher(wicketURL);
            if (rd != null) {
                // Need to use RequestDispatcher.include here otherwise internally rewinding on a redirect won't be
                // allowed (calling forward will close the response)
                rd.include(request, response);

                // process _other_ response states
                // check for redirects as a result of wicket processing the request
                ...
                if (redirectLocation != null) {
                    ...
                    // TODO: check if its redirect to wicket page
                    // (find _wu or_wuPortletMode or resourceId parameter)
                    ...
                    if (validWicketUrl) {
                        if (previousURL == null || previousURL.equals(redirectLocation)) {
                            ...
                            // internal Wicket redirection loop: unsure yet what to send out from here
                            /*
                             * TODO: determine what kind of error (message or page) should be written out
                        ...
                        if (responseState.isResourceResponse()) {
                             ...
                             * here...) if this will
                             * work.
                             * On Apache Pluto/Jetspeed-2, the above described
                             * handling *will* be
                            ...
                            // TODO what about this hardcoded DOCTYPE
                    ...
                    if (redirectLocation != null) {
                        ...
                        responseState.getWriter().write("<ajax-response><redirect><![CDATA["
                                + redirectLocation
    ...
    protected void processRequest(final PortletRequest request, final PortletResponse response, final PageType pageType)
        ...
        // core for processing
        wicketURL = getWicketURL(request, getDefaultPage(pageType));

        if (LOG.isDebugEnabled()) {
            LOG.debug("Portlet \"{}\" for wicket url: {}",
                    request.getAttribute(PortletRequest.LIFECYCLE_PHASE), wicketURL);
        }
     ...
     */
    @Override
    public void serveResource(final ResourceRequest request, final ResourceResponse response) throws PortletException,
            IOException {
    ...
    protected String fixWicketUrl(final String requestUrl, final String url) {
        if ((url != null) && (requestUrl != null) && (!ABSOLUTE_URI_PATTERN.matcher(url).matches())) {
            try {
                ...
                LOG.debug("Probably insignificant: problem with URL creation, URL will be fixed.");
            }
        }
     ...
     * <p/>
     * <p/>
     * Registers the default pages and their URLs for the different {@link PortletMode}s. Also corrects and slightly
     * incorrect URLs (see {@link #fixWicketUrl(String)}).
     * <p/>
     * <p/>
     * If no specific page was specified for a given portlet mode (VIEW, EDIT etc) then the page for that mode is set to
     * be the same page as that of the VIEW mode.
     * 
     * @param defaultPages
     * @see PortletMode
