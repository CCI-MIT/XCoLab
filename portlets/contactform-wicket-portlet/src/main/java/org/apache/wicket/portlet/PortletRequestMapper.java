...
/**
 * This class is the default request mapper for portlets. It modifies the generated URLs
 * (that are generated by the wrapped SystemMapper) depending on the type of the used request handler. Its purpose is to
 * implement all portlet related functionality from WebRequestCodingStrategy, that was used in Wicket 1.4.
 * 
        ...
        if (url != null) {
            ...
            if ((portletResponse != null) && (portletResponse instanceof MimeResponse)) {
                try {
                    ...
                    /*
                     * Resource URLs preserve all request render parameters (at least for Liferay even all POST
                     * parameters), so we have to remove all Wicket parameters (that have the portlet namespace),
                     * otherwise we would have all submitted values in the URL
                     */
        ...
        if (url != null) {
            ...
            if ((portletResponse != null) && (portletResponse instanceof MimeResponse)) {
                try {
                    ...
                    portletUrl.setParameter(WicketPortlet.WICKET_URL_PORTLET_PARAMETER
                            + ThreadPortletContext.getPortletMode().toString(), qualifiedPath);
                    qualifiedPath = portletUrl.toString();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
