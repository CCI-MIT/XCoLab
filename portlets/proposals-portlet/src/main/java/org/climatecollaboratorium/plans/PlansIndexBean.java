...
public class PlansIndexBean implements Serializable {
	...
	private List<TestTab> testTabs = Arrays.asList(new TestTab("one"),
			new TestTab("two"), new TestTab("three"));
	...
	private enum Mode {
		PROPOSALS_ALL("All proposals"), PROPOSALS_USER_OWNS("My proposals",
				true), PROPSALS_USER_SUPPORTS("Proposals I support", true), PROPOSALS_OPEN(
				"Open proposals"), PROPOSALS_NEED_ASSISTANCE(
				"Proposals seeking help");
	...
	public PlansIndexBean(ContestPhaseWrapper contestPhaseWrapper)
			throws PortalException, SystemException {
		this.contestPhase = contestPhaseWrapper;
		/*
		 * if (contestPhase.getStatus() == ContestStatus.VOTING) {
		 * defaultSortColumn = Columns.VOTES; } else { defaultSortColumn =
		 * Columns.SUPPORTERS; }
		 */
		defaultSortColumn = ContestPhaseHelper
				.getDefaultSortPhaseColumn(contestPhase.getPhase());
		...
		/*
		 * if (contestPhaseWrapper != null) { getPlans(); }
		 */

	}

	public void init(ContestPhaseWrapper currentPhase, NavigationEvent event)
			throws PortalException, SystemException {
		...
		if (contestPhase != null
				&& contestPhase.getPhaseId().equals(currentPhase.getPhaseId())) {
			// ignore
		} else {
		...
		if (params != null && params.containsKey(VIEW_PROPOSALS_PARAM)) {
			boolean tmp = params.get(VIEW_PROPOSALS_PARAM).equals(
					PREVIOUS_PROPOSALS);
			if (tmp != showPreviousProposals) {
				...
				if (showPreviousProposals) {
					queryPhases.clear();
					queryPhases.addAll(ContestPhaseLocalServiceUtil
							.getPreviousPhases(contestPhase.getPhase()));
				}
			}
		} else {
	...
	private void initSortColun() {
		 ...
		 * contestPhase.getStatus() == ContestStatus.VOTING) { sortColumn =
		 * Columns.VOTES.name(); } else { sortColumn =
		 * Columns.SUPPORTERS.name(); }
		 */
		sortAscending = false;
	}
	...
	public List<ContestPhaseWrapper> getContestPhases() {
		List<ContestPhaseWrapper> result = contestPhase == null ? Collections
				.<ContestPhaseWrapper> emptyList() : contestPhase.getContest()
				.getPhases();
		return result;
	}

	public int getContestPhaseIndex() {
		int result = contestPhase == null ? -1 : contestPhase.getContest()
				.getPhases().indexOf(contestPhase);
		return result;
	}

	/*
	 * public void setContestPhaseIndex(int i) throws SystemException,
	 * PortalException { if (contestPhase!=null)
	 * init(contestPhase.getContest().getPhases().get(i).getPhaseId(),
	...
	private void bindEvents() {
		...
		handlerRegistrations.add(eventBus.registerHandler(
				PlanDeletedEvent.class, new EventHandler<PlanDeletedEvent>() {
		...
		handlerRegistrations.add(eventBus.registerHandler(
				PlanCreatedEvent.class, new EventHandler<PlanCreatedEvent>() {
	...
	public List<PlanIndexItemWrapper> getPlansMultiContestPage()
	...
	public List<PlanIndexItemWrapper> getPlans() throws SystemException,
			PortalException {
		if (contestPhase == null) {
			return null;
		}
		...
		if (updatePlansList) {
			...
			// if contest isn't active then we should sort by votes
			/*
			 * if (!contestPhase.getContest().isContestActive()) { sortAttribute
			 * = Attribute.VOTES.name(); }
			 */
			if (sortCol.isSortable()) {
				sortAttribute = sortCol.getSortAttribute().name();
			}
			if (showPreviousProposals) {
				notFilteredPlans = PlanItemLocalServiceUtil.getPlans(ectx
						.getSessionMap(), ectx.getRequestMap(), null,
						queryPhases, 0, 1000, sortAttribute,
						sortAscending ? "ASC" : "DESC", false);
			} else {
				notFilteredPlans = PlanItemLocalServiceUtil.getPlans(
						ectx.getSessionMap(), ectx.getRequestMap(), 0L,
						contestPhase.getPhase().getContestPhasePK(), 0, 1000,
						sortAttribute, sortAscending ? "ASC" : "DESC", false);
			}
			...
			final Long userId = Helper.isUserLoggedIn() ? Helper
					.getLiferayUser().getUserId() : -1;
			for (Iterator<PlanItem> i = notFilteredPlans.iterator(); i
					.hasNext();) {
				...
				if (PlanItemLocalServiceUtil.getRibbon(plan) != null
						&& PlanItemLocalServiceUtil.getRibbon(plan) > 0) {
					plansWithRibbons.add(piiw);
				} else if (PlanItemLocalServiceUtil.getTags(plan) != null) {
					plansFeatured.add(piiw);
				} else if (PlanItemLocalServiceUtil.getName(plan).startsWith(
						"Untitled")) {
			...
			if (defaultSort) {
				...
				Collections.sort(plansWithRibbons,
						new Comparator<PlanIndexItemWrapper>() {

							@Override
							public int compare(PlanIndexItemWrapper o1,
									PlanIndexItemWrapper o2) {
								try {
				...
				// update sorting for sections
				Collections.sort(plansFeatured,
						new Comparator<PlanIndexItemWrapper>() {
	...
	public void updateSortColumn(ActionEvent e) {
		defaultSort = false;
		String newSortColumn = (String) e.getComponent().getAttributes()
				.get("sortColumn");
		// this.sortColumn =
	...
	public ConfigureColumnsBean getColumnsConfiguration()
			throws SystemException, PortalException {
	...
	private void columnsUpdate() throws PortalException, SystemException {
		ExternalContext ectx = FacesContext.getCurrentInstance()
				.getExternalContext();
		// plansUserSettings =
		// PlansUserSettingsLocalServiceUtil.getPlanUserSettings(ectx.getSessionMap(),
		// ectx.getRequestMap(), contestPhase.getPlanType());
		columns = new ArrayList<Columns>();
		for (Columns col : ContestPhaseHelper.getPhaseColumns(contestPhase
				.getPhase())) {
			columns.add(col);
		}
		updatePlansList = true;
	...
	public int getVotesCount() throws SystemException, PortalException {
		return PlanVoteLocalServiceUtil.countPlanVotes(contestPhase
				.getPlanType());
	}
	...
	public void findUserVote(ActionEvent e) throws PortalException,
			SystemException {
		findUserVote = true;
		refresh();
	}
	...
	public boolean getHasUserVoted() throws PortalException, SystemException {
		if (Helper.isUserLoggedIn()) {
			try {
				PlanVote vote = PlanVoteLocalServiceUtil.getPlanVote(Helper
						.getLiferayUser().getUserId(), contestPhase
						.getContest().getContestId());
				return vote != null;
	...
	public void setSelectedMode(String name) throws SystemException,
			PortalException {
	...
	public void setCurrentMode(Mode mode) throws SystemException,
			PortalException {
	...
	public List<SelectItem> getAvailableModes() {
		if (availableModes == null) {
			...
			for (Mode m : Mode.values()) {
				if (m.requiresLogin && !Helper.isUserLoggedIn())
					continue;
	...
	// }
	//
	// public void setShowProposalsThatUserOwns(boolean show) throws
	// PortalException, SystemException {
	// showProposalsThatUserOwns = show;
	// }
	//
	// public void setShowProposalsThatUserSupports(boolean show) throws
	// PortalException, SystemException {
	// showProposalsThatUserSupports = show;
	// }
	//
	// public void setShowProposalsThatNeedSupporters(boolean show) throws
	// PortalException, SystemException {
	// showProposalsThatNeedSupporters = show;
	// }
	//
	// public void setShowProposalsThatAreOpen(boolean show) throws
	// PortalException, SystemException {
	...
	public List<PlanIndexItemWrapper> getPlansWithRibbons()
			throws SystemException, PortalException {
		getPlans();
		return plansWithRibbons;
	}

	public List<PlanIndexItemWrapper> getPlansFeatured()
			throws SystemException, PortalException {
		getPlans();
		return plansFeatured;
	}

	public List<PlanIndexItemWrapper> getPlansWithProperName()
			throws SystemException, PortalException {
		getPlans();
		return plansWithProperName;
	}

	public List<PlanIndexItemWrapper> getPlansUntitled()
			throws SystemException, PortalException {
		getPlans();
