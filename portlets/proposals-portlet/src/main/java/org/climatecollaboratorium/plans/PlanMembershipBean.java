...
public class PlanMembershipBean implements Serializable {
	/**
	 * 
	 */
	...
	public PlanMembershipBean(PlanItem plan, PlanBean planBean,
			PlansPermissionsBean permissions) throws PortalException,
			SystemException {
		this.plan = plan;
	...
	public List<PlanMember> getPlanMembersHalf1() throws SystemException {
		 ...
		 * planMembersHalf1.addAll(getPlanMembersSublist(0,
		 * (plan.getMembers().size() / 2) + (plan.getMembers().size() % 2))); }
		 * return planMembersHalf1;
		 */
		return getPlanMembersSublist(
				0,
				(PlanItemLocalServiceUtil.getMembers(plan).size() / 2)
						+ (PlanItemLocalServiceUtil.getMembers(plan).size() % 2));
	}
	...
	public List<PlanMember> getPlanMembersHalf2() throws SystemException {
		 ...
		 * planMembersHalf2.addAll(getPlanMembersSublist
		 * ((plan.getMembers().size() / 2) + (plan.getMembers().size() % 2),
		 * plan.getMembers().size())); } return planMembersHalf2;
		 */
		return getPlanMembersSublist((PlanItemLocalServiceUtil.getMembers(plan)
				.size() / 2)
				+ (PlanItemLocalServiceUtil.getMembers(plan).size() % 2),
				PlanItemLocalServiceUtil.getMembers(plan).size());
	}

	private List<PlanMember> getPlanMembersSublist(int from, int to)
			throws SystemException {
		List<PlanMember> ret = new ArrayList<PlanMember>();
		for (int i = from; i < to; i++) {
			ret.add(new PlanMember(plan, PlanItemLocalServiceUtil.getMembers(
					plan).get(i), this, permissions));
		}
		return ret;
	}

	public List<PlanMembershipRequest> getPlanMembershipRequests()
			throws SystemException, PortalException {
		planMembershipRequests.clear();
		for (MembershipRequest request : PlanItemLocalServiceUtil
				.getMembershipRequests(plan)) {
			planMembershipRequests.add(new PlanMembershipRequest(request, plan,
					planBean, permissions));
		}
		return planMembershipRequests;
	}

	public void requestMembership(ActionEvent e) throws PortalException,
			SystemException, AddressException, MailEngineException {
		if (Helper.isUserLoggedIn()) {
			PlanItemLocalServiceUtil.addMembershipRequest(plan, Helper
					.getLiferayUser().getUserId(),
					comment.length() == 0 ? "No comments" : comment);
			planBean.refresh();
			...
			String proposalUrl = String.format(PROPOSAL_URL,
					requestUrl.substring(0, requestUrl.indexOf("/", 10)),
					PlanItemLocalServiceUtil.getContest(plan).getContestPK(),
					plan.getPlanId());
			String subject = String.format(MSG_MEMBERSHIP_REQUEST_SUBJECT,
					Helper.getLiferayUser().getFullName(),
					PlanItemLocalServiceUtil.getName(plan));
			String content = String.format(MSG_MEMBERSHIP_REQUEST_CONTENT,
					Helper.getLiferayUser().getFullName(),
					PlanItemLocalServiceUtil.getName(plan), proposalUrl);

			MessageUtil.sendMessage(subject, content, Helper.getLiferayUser()
					.getUserId(), Helper.getLiferayUser().getUserId(),
					receipients, null);
		}
	}
	...
	public void toggleRequestingOrJoin(ActionEvent e) throws SystemException,
			PortalException {
		if (PlanItemLocalServiceUtil.getOpen(plan)) {
			PlanItemLocalServiceUtil.joinIfNotAMember(plan, Helper
					.getLiferayUser().getUserId());
			planMembers.clear();
		} else {
	...
	public List<SelectItem> getAvailableUserRoles() throws SystemException,
		...
		if (permissions.getCanAdminAll()) {
			items.add(new SelectItem(PlanUserPermission.OWNER,
					PlanUserPermission.OWNER.getDescription()));
		}
		if (permissions.getCanAdmin()) {
			items.add(new SelectItem(PlanUserPermission.MEMBER,
					PlanUserPermission.MEMBER.getDescription()));
			items.add(new SelectItem(PlanUserPermission.ADMIN,
					PlanUserPermission.ADMIN.getDescription()));
		}
		return items;
	...
	public void removeCurrentUser(ActionEvent e) throws SystemException,
		...
		if (Helper.isUserLoggedIn()) {
			PlanItemLocalServiceUtil.removeMember(plan, Helper.getLiferayUser()
					.getUserId(), Helper.getLiferayUser().getUserId());
